     ...
     if (solenoid_active_M == false) {
       ...
       // (PORT LIGHT ON) Turn on Left LED if it is a valid trial
         if ((valid_response_window) && (led_state_L == LOW)){
           ...
           // RESET PORT COUNTER to count # of incorrect pokes during response window 
           left_port_counter = 0;
           mid_port_counter = 0;   
        ...
        if ((reward_window) && (reward_type == 1)) {
           ...
           if (start_iti_window) {
              ...
              if ((millis() - iti_start_time) >= iti_interval[random_idx]) {
                ...
                count = count + 1;      // count keeps track of the number of FC trials administered; makes first 10 trials FC 
