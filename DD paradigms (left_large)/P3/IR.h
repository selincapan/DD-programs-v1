  ...
  if (read_gate_M != ir_previous_state_M) {
    if (read_gate_M == LOW) {
      ...
      if (initiation_window){
        ...
        // Prints Out number of invalid pokes made when the middle port is blinking (before trial initiation) 
        ...
        valid_response_window = true; // start of valid trial (where the animal will poke (i.e respond) to get reward)

        // reset all counters once valid_response_window starts (is true)
        // allows me to count the "valid" pokes after trial initiation (during valid response window)
