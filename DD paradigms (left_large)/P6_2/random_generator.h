...
//randomizes the list of trial types 
...
int random_port(){
   ...
   if (get_random_port and count < 10) {  //if it's within the first 10 trials 
    ...
    //selects a trial type from list of trial types (shuffled); allows exactly 2/20 trials to be FC
      ...
      trial_type_idx = trial_type_idx + 1;        //goes through each index of the randomized array 
      
      if (trial_type_idx == 20) {                 //if we have gone through the array, re-shuffle it and start from the beginning 
