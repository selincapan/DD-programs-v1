  ...
  if (read_gate_M != ir_previous_state_M) {
    if (read_gate_M == LOW) {
      ...
      if (initiation_window){
        ...
        // Prints Out number of invalid pokes made during pre-trial phase (middle port blinking)
        ...
        // # of poke counts during initation window (before VALID response window)
