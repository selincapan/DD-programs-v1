  ...
  if (read_gate_M != ir_previous_state_M) {
    if (read_gate_M == LOW) {
      ...
      if (initiation_window){
        ...
        // Prints Out number of invalid pokes made during pre-trial phase (middle light blinking) 
        ...
        initiation_window = false;    // end of trial window
        valid_response_window = true; // start of response window 
