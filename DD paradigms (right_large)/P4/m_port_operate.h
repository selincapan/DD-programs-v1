...
void activate_middle_led() {
     ...
     if (solenoid_active_M == false) {
       ...
       // (PORT LIGHT ON) Turn on both LEDs if it is a valid trial
         ...
         // BOTH LIGHTS ON, poke in RIGHT PORT --> large reward port! 
         if ((valid_response_window) && (led_state_L == HIGH) && (led_state_R == HIGH) && (poke_in_R) && (right_port_counter > 0)) {
           ...
           Serial.print(F("91170:"));    // Timestamp for free choice Valid Light Off (indicates a poke in which port led to reward) 
         ...
         // BOTH LIGHTS ON, poke in LEFT PORT  --> small reward port! 
